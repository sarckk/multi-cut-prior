'''
This file is a modified version of https://github.com/nik-sm/generator-surgery/blob/master/recover.py and https://github.com/nik-sm/generator-surgery/blob/master/mgan.py
'''
import argparse
import os
import shutil
...
from utils import (get_z_vector, load_target_image, psnr, psnr_from_mse)
import wandb
import logging
import lpips
...
# Total variation loss implementation from github user rrmina
...
def _recover(x,
    ...
    for j in trange(n_steps,
        ...
        packed_losses = pack_losses(forward_model, x_hats_clamp, x, y_observed, y_masked_part)
        ...
        if writer is not None:
            for k,v in packed_losses.items():
    ...
    if packed_losses['TRAIN_MSE'] > 0.01:
        is_valid_run = False

    return x_hats_clamp.squeeze(), y_observed.squeeze(), packed_losses, saved_params, is_valid_run
